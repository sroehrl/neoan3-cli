...
let CredentialHandler = {
    ...
    init: () => {
        ...
        let credentialObj = fs.existsSync(credentialsFolder +
            '/credentials.json');
        if (credentialObj) {
            CredentialHandler.credentialObj = fs.readJsonSync(credentialsFolder
                + '/credentials.json');
        }
    ...
    topLevel: () => {
        ...
        let questions = [
            ...
            },
            {
                type: 'input',
                name: 'name',
                message: 'How do you want to call ' +
                    'these credentials?', when: (answer) => {
        ...
        if (existingProperties.length > 0) {
            ...
            existingProperties.forEach(property => {
                console.log('   ' + property + ': ' +
                    CredentialHandler.credentialObj[topic][property]);
            });
            console.log('\n#####################################\n');
        }
