...
const transformer = {
    check(name){
        ...
        if(fs.existsSync('./model/' + stringHelper.flcase(name) + '/' + stringHelper.fucase(name) + '.transformer.php')){
            console.log('Transformer already exists. Don\'t want to overwrite...');
            process.exit();
        }
        return true;
    },
